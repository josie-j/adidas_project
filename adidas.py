...
with tab3:
    ...
    with c5:
        ...
        method_stats = filtered.groupby("Sales Method").agg({"Profit Rate": "mean"}).sort_values("Profit Rate", ascending = False)
        st.bar_chart(method_stats, use_container_width= True)

    with c6:
        st.markdown("#### 판매방법별 평균 단가")
        method_stats2 = filtered.groupby("Sales Method").agg({"Price per Unit": "mean"}).sort_values("Price per Unit", ascending= False)
        st.bar_chart(method_stats2, use_container_width= False)
...
- **필터**: 지역, 소매업체, 제품, 팬마방법별로 상세 분석이 가능합니다.
